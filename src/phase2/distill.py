...
def teacher_encode(model: nn.Module, x: DataLoader, kind: str, post_gap: bool = True) -> torch.Tensor:
    ...
    if kind == "resnet50":
        ...
        if post_gap:
            x = model.avgpool(x)
            x = model.classifier[0](x) # LayerNorm para manter a consistência com o que o student espera (features pós-GAP)
    elif kind == "vgg16":
        x = model.features(x)
        if post_gap:
...
def eval_distill_mse(teacher_model: TeacherModel, student: Student,  loader, kind: str, device: torch.device) -> float:
    ...
    for x, _ in loader:
        ...
        loss = F.mse_loss(fs.float(), ft.float())
...
def destill_students(teacher: TeacherModel, student: Student, train_loader, validation_loader,
                     ...
                     encoder_lr: float = 1e-5, classifier_lr: float = 1e-3,
                     weight_decay: float = 1e-2, T: float = 4.0, alpha: float = 0.8):
    ...
    # LRs separados: o encoder já vem destilado (encoder_lr menor), o classificador
    # parte do zero (classifier_lr maior). Param group 0 = encoder, 1 = classifier.
    encoder_parameters = [param for name, param in student.named_parameters() if "classifier" not in name]
    ...
    best_val_acc = -1.0
    runs_with_best_val_acc = 0

    best_loss = float("inf")
    runs_with_best_loss = 0
    ...
    for epoch in range(epochs):
        ...
        for x, y in train_loader:
            ...
            with torch.amp.autocast("cuda", dtype=torch.float16):
                ...
                loss_kd = F.kl_div(
                    F.log_softmax(student_pred.float() / T, dim=1),
                    F.softmax(teacher_pred.float() / T, dim=1),
                    reduction="batchmean"
                ) * (T * T)
        ...
        enc_lr, cls_lr = scheduler.get_last_lr()
        print(f"{kind}] epoch {epoch:02d} | train_loss {train_loss:.5f} | train_acc {train_acc:.4f} | "
              f"val_loss {loss_val:.5f} | val_acc {val_acc:.4f} | enc_lr {enc_lr:.2e} | cls_lr {cls_lr:.2e}")
        ...
        if val_acc > best_val_acc:
            ...
            runs_with_best_val_acc += 1

        if loss_val < best_loss:
            runs_with_best_loss = 0
            best_loss = loss_val
        else:
            runs_with_best_loss += 1


        if runs_with_best_val_acc > 5 and runs_with_best_loss > 5:
            print(f"Early stopping com val_acc: {best_val_acc} (menor loss = {best_loss:.5f})")
            break
